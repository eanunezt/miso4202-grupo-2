...
module.controller('CategoriaCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
    ...
    $scope.guardar = function () {
        ...
        if (error)
            return;
        $http.post('./rest/categoria', JSON.stringify($scope.datosFormulario), {}
            ).success(function (data, status, headers, config) {
    ...
    $scope.eliminar = function (data){
        if (confirm('\xbfDesea elminar este registro?')) {    
            $http.delete('./rest/categoria/'+data.id, {})
                .success(function (data, status, headers, config) {
                    $scope.listar();
                }).error(function (data, status, headers, config) {    
                    alert('Error al eliminar la informaci\xf3n de Categoria, por favor intente m\xe1s tarde');
            });   
        }
    };
}]);
