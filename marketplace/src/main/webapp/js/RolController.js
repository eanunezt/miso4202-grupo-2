...
module.controller('RolCtrl', ['$scope', '$filter', '$http', function ($scope, $filter, $http) {
        ...
        $scope.listarUsuario=function(){
            $http.get('./rest/usuario', {})
                .success(function (data, status, headers, config) {
                    ...
                    alert('Error al consultar la informaci\xf3n de usuario, por favor intente m\xe1s tarde');
            });    
        };
        $scope.listarUsuario();
    ...
    $scope.guardar = function () {
        ...
        if (error)
            return;
        $http.post('./rest/rol', JSON.stringify($scope.datosFormulario), {}
            ).success(function (data, status, headers, config) {
